...
`type = "tab"` and `type = "split"` mirror the built-in new-tab and split
actions but run the command instead of the default shell. They create regular,
persistent panes that follow the `[terminal] new_cwd` policy. Session restore
keeps the pane layout but reopens these panes with the default shell.
`type = "split"` accepts an optional `direction` of `"right"` (default) or
`"down"`. On Unix and macOS, use `exec` so the command replaces the wrapper
shell, for example to open an alternate shell without changing
`default_shell`; on Windows, use a plain shell command such as
`command = "pwsh.exe -NoLogo"` instead:
...
- `$name` — custom pane metadata named `name`. Herdr fills `$repo` and `$worktree` itself when the pane's foreground process runs in a different repository or linked worktree than its workspace (`wt:<checkout>`); a value reported by the pane wins.
...
- `workspace` — workspace name.
- `worktree` — `wt:` followed by the checkout directory name when the foreground process of the first tab's root pane runs inside a linked Git worktree; omitted on the primary checkout.
...
With one repository, the `worktree` token adds distinct linked checkouts from the active tab's other panes below the configured rows; the root pane's branch row stays in place. If the active tab spans repositories, the Space groups worktrees under project names and shows a project count in place of an automatic Space name. Custom Space names remain unchanged.

Left-click any row below the first one to fold a Space entry down to its first row; a `…` marker appears at the right edge and clicking it unfolds the entry. In navigate mode, Space toggles the fold of the selected Space. Fold state persists per client alongside collapsed worktree groups.
