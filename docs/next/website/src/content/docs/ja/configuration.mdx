...
`type = "tab"` と `type = "split"` は、組み込みの新規タブ・分割アクションと同じ動作をしつつ、デフォルトシェルの代わりにコマンドを実行します。作成されるのは通常の永続的なペインで、`[terminal] new_cwd` ポリシーに従います。セッション復元ではペインの配置は保たれますが、これらのペインはデフォルトシェルで開き直されます。`type = "split"` は任意で `direction` に `"right"`（デフォルト）または `"down"` を指定できます。`default_shell` を変えずに別のシェルを開くには、Unix と macOS では `exec` でラッパーシェルを置き換えてください。Windows では代わりに `command = "pwsh.exe -NoLogo"` のようなシェルコマンドを使ってください:
...
- `$name` — `name` という名前のカスタムペインメタデータ。ペインのフォアグラウンドプロセスがワークスペースと異なるリポジトリや linked worktree で動いている場合、Herdr が `$repo` と `$worktree`（`wt:<checkout>`）を自動で埋めます。ペインが報告した値が優先されます。
...
- `workspace` — ワークスペース名。
- `worktree` — 最初のタブのルートペインのフォアグラウンドプロセスが linked worktree の中にある場合、その checkout ディレクトリ名を `wt:` 付きで表示。primary checkout では表示されません。
- `branch` — 利用可能な場合は Git ブランチ。
- `git_status` — 0 でない場合は Git の ahead と behind の件数。
- `$name` — `name` という名前のカスタムワークスペースメタデータ。

同じリポジトリの別の worktree がアクティブなタブのペインにあれば、`worktree` トークンは設定した行の下に重複なしで追記します。ルートペインのブランチ行はそのまま残ります。異なるリポジトリが同じタブにある場合は、プロジェクト名ごとに worktree をまとめ、自動名の Space にはプロジェクト数を表示します。手動で付けた Space 名は変わりません。

1 行目より下の行を左クリックすると、Space エントリを 1 行目だけに畳めます。畳んだエントリは右端に `…` が出て、そこをクリックすると開きます。navigate モードでは Space キーで選択中の Space の畳みを切り替えます。畳み状態は collapsed worktree group と同じくクライアントごとに保存されます。
