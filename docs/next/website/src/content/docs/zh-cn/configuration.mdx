...
- `$name` — 名为 `name` 的自定义窗格元数据。当窗格的前台进程运行在与其工作区不同的仓库或 linked worktree 中时，Herdr 会自动填充 `$repo` 和 `$worktree`（`wt:<checkout>`）；窗格自行报告的值优先。
...
- `workspace` — 工作区名称。
- `worktree` — 当第一个标签页根窗格的前台进程位于 linked Git worktree 中时，以 `wt:` 前缀显示该 checkout 目录名；在 primary checkout 上不显示。
- `branch` — Git 分支（如有）。
- `git_status` — 非零时显示 Git ahead 和 behind 数量。
- `$name` — 名为 `name` 的自定义工作区元数据。

同一仓库的其他 linked worktree 若出现在活动标签页的窗格中，`worktree` token 会在已配置的行之后追加不重复的 checkout；根窗格的分支行保持原位。若活动标签页包含不同仓库，Space 会按项目名称分组显示 worktree，并用项目数量替代自动生成的 Space 名称。手动设置的 Space 名称不变。

左键点击第一行以下的任意一行，可将 Space 条目折叠为仅显示第一行；折叠后右端会出现 `…` 标记，点击它即可展开。在 navigate 模式下，按 Space 键可切换所选 Space 的折叠状态。折叠状态与 collapsed worktree group 一样按客户端保存。
